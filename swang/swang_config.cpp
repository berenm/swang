...
namespace llvm {
  namespace yaml {
    ...
    template< >
    struct ScalarEnumerationTraits< swang::language_type > {
    ...
    struct MappingTraits< swang::config > {
      static void mapping(IO& io, swang::config& style) {
        ...
        io.mapOptional("exception_variable", style.exception_variable_style);
        ...
        io.mapOptional("lambda", style.lambda_style);
        io.mapOptional("interface", style.interface_style);
...
namespace swang {
  ...
  swang::config get_config(std::string name, std::string file_name, std::string fallback_name) {
    ...
    for (llvm::StringRef directory = path; !directory.empty(); directory = llvm::sys::path::parent_path(directory)) {
      ...
      if (is_file) {
        auto buffer = llvm::MemoryBuffer::getFile(config_file.c_str());
        if (buffer.getError()) {
